...
export default function LessonPage({ params }: { params: Promise<{ slug: string; lessonSlug: string }> }) {
  ...
  const client = useQueryClient(); const { data: lesson, isLoading } = useQuery({ queryKey: ["lesson", lessonSlug], queryFn: async () => (await api.get(`/courses/${slug}/lessons/${lessonSlug}`)).data });
